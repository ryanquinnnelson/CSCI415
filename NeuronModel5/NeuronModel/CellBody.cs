    ...
    public class CellBody
    {
        ...
        public int TryRemoveFromBuffer() //tested
        {
            ...
            if (state == 0)
            {
                ...
                //Console.WriteLine("Membrane Potential: " + current);

                //save result as output
                StoreAsOutput(current, DateTime.Now);
        ...
        private void ClearBuffer()
        {
            ...
            //clear out all entries from buffer
        ...
        private void PerformMembranePotentialSpike() //tested
        {
            ...
            StoreAsOutput(DEPOLARIZATION_SPIKE, DateTime.Now);
            Thread.Sleep(1);

            current = Interlocked.Exchange(ref membranePotential, HYPERPOLARIZATION_SPIKE);
            StoreAsOutput(HYPERPOLARIZATION_SPIKE, DateTime.Now);
        ...
        private void StoreAsOutput(int potential, DateTime when) //tested
        {
            TimeSpan ts = when.Subtract(start);
